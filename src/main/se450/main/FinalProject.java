...
/**
 * The FinalProject class sets up the window frames for the game waiting for the start key to be pressed, and this class is also an observer of 
 * the Score and Motion classes, therefore the shapes would be perfectly updated and displayed during the game, and the score on the panel would 
 * be updated simultaneously when shapes got shot or ship got hit. 
 * @author wenwenzhang
 *
 */
public class FinalProject extends JFrame implements IScoreObservable
{
	...
	private int totalScore;//remove this and the IObservable from this class - its for example only
	...
	public FinalProject()
	{
		...
		//setSize(1440, 840);//set the size of the window to 1440 x 840
		
		DisplayManager.getDisplayManager().setDisplaySize(ConfigurationManager.getConfigurationManager().getConfiguration().getWidth(), 
				ConfigurationManager.getConfigurationManager().getConfiguration().getHeight());
		setSize(DisplayManager.getDisplayManager().getWidth(), DisplayManager.getDisplayManager().getHeight());
	...
	/**
	 * update the scores on the panel.
	 */
	@Override 
	public void update()
	{
	...
	/**
	 * Load configuration from the json file using the ConfigurationParser class.
	 */
	...
	/**
	 * Set the realtime score points from the Score class to the totalScore variable.
	 */
	@Override
	public void score() 
	{
